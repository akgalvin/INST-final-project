...
# This block asks and checks that the script is only used with only one command-line argument. If the user does not abide by this,
## it will raise a value error to show that there is only one argument allowed
if len(sys.argv) != 2:
    # Raise an error and provide a helpful error message to the user
    raise ValueError("Usage: python script.py <file_path>")

##This block will receive the file path from the argument in the command line.
##The user is meant to provide their own file at the sys.argv line. In this case
## my file path is /Users/annagalvin/Desktop/INST Final Project/Directory 1/Text File 1
